...
public class FlightController {
	...
	// for flights search 
	// GET = http://localhost:9090/aerocom/api/admin/flightSearch/{from_location}/{to_location}/{date}
    @GetMapping("/admin/flightSearch/{from_location}/{to_location}/{date}")
	public List<Flight>getFlightData(@PathVariable(value="from_location") String from_location, @PathVariable(value ="to_location") String to_location
			, @PathVariable("date") String date) {
    	
		return (List<Flight>)frepo.findByFromLocationAndToLocationAndDate(from_location,to_location, date);
    }
    
    //GET -  http://localhost:9090/aerocom/api/admin/findFlight/{flight_number}...//only for admin
    @GetMapping("/admin/findFlight/{flightNumber}")
    public ResponseEntity<Flight> getProductById(@PathVariable(value="flightNumber") String flightNumber )throws ResourceNotFoundException{
    	Flight flight = frepo.findByFlightNumber(flightNumber).orElseThrow(()-> new ResourceNotFoundException("Flight not found: "+flightNumber));
    ...
    public Map<String,Boolean> deleteFlight(@PathVariable(value="id") String fId)
     throws ResourceNotFoundException
     {
     Flight flight=frepo.findById(fId).orElseThrow(() -> new ResourceNotFoundException
 			("Flight Not Found for this Id: " +fId));
